#!/usr/bin/env bash
# run-evaluation.sh — SPIKE-006 multi-turn classification evaluation
# Forked from SPIKE-004, with parse-retry logic and improved system prompt.
#
# Usage:
#   ./run-evaluation.sh --model <MODEL_ID> --clone-dir <PATH> [--max-turns N] [--verbose]
#
...
# Results saved to results-multiturn/<model-short-name>/<repo>.txt (full prose).
#
# Improvements over SPIKE-004:
#   - Parse retry: if YAML/JSON parse fails, asks model to reformat (up to 2 retries)
#   - Improved system prompt with strict examples and JSON fallback
...
SYSTEM_PROMPT="${SCRIPT_DIR}/spike-006-system-prompt.md"
...
RESULTS_DIR="${SCRIPT_DIR}/results-multiturn/${MODEL_SHORT}"
mkdir -p "$RESULTS_DIR"
...
# Fallback 2: try bare JSON object (entire response is JSON)
if result is None:
    stripped = text.strip()
...
# ── is_parse_empty: check if parsed frontmatter is empty {} ───────────────────
is_parse_empty() {
  local parsed="$1"
...
retry_parse() {
  local raw_response="$1" retry_num="$2"
...
\`\`\`json
{\"verdict\": \"classified\", \"primary_style\": \"StyleName\", \"secondary_styles\": [\"Style2\"], \"confidence\": 0.85}
\`\`\`

Your analysis text here...

IMPORTANT: Each YAML key must appear exactly once. Do not write 'type: type: file' — write 'type: file'." | llm -m "$MODEL" -c 2>/dev/null || echo "ERROR: llm retry call failed")
  echo "$retry_response"
}
...
process_repo() {
  ...
  while is_parse_empty "$parsed" && [[ $parse_retries -lt $MAX_PARSE_RETRIES ]]; do
    ...
    response=$(retry_parse "$response" "$parse_retries")
    parsed=$(parse_yaml_frontmatter "$response")
  done

  if is_parse_empty "$parsed"; then
    log "  $repo_name: FAILED — could not parse response after $parse_retries retries"
    local out_file="$RESULTS_DIR/${repo_name}.txt"
    {
      echo "# $repo_name — $MODEL"
      echo "# Turns: $turn"
      echo "# Parse retries: $parse_retries"
  ...
  verbose "  Turn $turn: $response_type (parse_retries=$parse_retries)"
...
Please classify this repository now." | llm -m "$MODEL" -c 2>/dev/null || echo "ERROR: llm call failed")
    ...
    # Parse retry loop for subsequent turns
    parsed=$(parse_yaml_frontmatter "$response")
    while is_parse_empty "$parsed" && [[ $parse_retries -lt $MAX_PARSE_RETRIES ]]; do
      parse_retries=$((parse_retries + 1))
      response=$(retry_parse "$response" "$parse_retries")
    ...
    if is_parse_empty "$parsed"; then
      log "  $repo_name: FAILED on turn $turn — could not parse after $parse_retries retries"
      break
    fi
  ...
  local out_file="$RESULTS_DIR/${repo_name}.txt"
  {
    echo "# $repo_name — $MODEL"
    echo "# Turns: $turn"
    echo "# Parse retries used: $parse_retries"
    echo "# Date: $(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo ""
    echo "$response"
  } > "$out_file"

  log "  $repo_name: done (turns=$turn, parse_retries=$parse_retries) → $out_file"
}

# ── Main ──────────────────────────────────────────────────────────────────────
main() {
  log "SPIKE-006 Multi-Turn Evaluation (with parse retry)"
