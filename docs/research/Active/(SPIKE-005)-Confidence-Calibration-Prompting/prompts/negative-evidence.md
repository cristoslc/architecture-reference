...
confidence: <0.0-1.0>
counter_evidence:
  alternative_style: <most plausible alternative>
  evidence_that_would_change_mind: "<specific evidence description>"
  plausibility_of_counter: <low|medium|high>
---
...
IMPORTANT: After your classification analysis, you MUST include a section titled
"## Counter-evidence analysis" where you:
1. Name the most plausible alternative primary style
2. Describe specifically what evidence would make you change your classification
3. Assess how likely it is that such evidence exists (low/medium/high)
4. Use this assessment to set your confidence:
   - If counter-evidence is HIGHLY plausible (you suspect it might exist): confidence 0.65-0.75
   - If counter-evidence is MODERATELY plausible (possible but you'd be surprised): confidence 0.76-0.85
   - If counter-evidence is LOW plausibility (you'd be very surprised): confidence 0.86-0.92
   - If counter-evidence is IMPLAUSIBLE (would contradict multiple observed facts): confidence 0.93-0.95
>
```

If there are no secondary styles, use `secondary_styles: []`.

### Confidence through counter-evidence (MANDATORY)

Do NOT assign confidence directly. Instead, follow this process:

1. **Classify** the repository as you normally would
2. **Identify the strongest alternative** — what is the most credible different primary style someone could argue for?
3. **Describe the counter-evidence** — what specific files, configs, or patterns would you need to find to change your primary classification? Be concrete: "finding a shared database connection pool used by all services" not "finding evidence of a different style"
4. **Rate the plausibility** of that counter-evidence actually existing in this codebase:
   - **high**: You have specific reasons to suspect this evidence might exist (e.g., you saw hints but didn't explore further, or the codebase has characteristics consistent with both styles)
   - **medium**: The counter-evidence is theoretically possible but you saw no hints of it
   - **low**: The counter-evidence would contradict things you already observed
5. **Derive confidence** from the plausibility rating using the scale above

This process ensures confidence reflects genuine uncertainty rather than a reflexive high number.
...
6. **Derive confidence from counter-evidence analysis.** Follow the mandatory process above. Never assign confidence without first identifying what would change your mind.
