...
```bash
# Compute current content hash (file paths + contents, excluding .source.yml)
CURRENT=$(cd .agents/skills && find code-review -type f ! -name '.source.yml' -print0 \
  | sort -z | while IFS= read -r -d '' f; do printf '%s\n' "$f"; cat "$f"; done \
  | sha256sum | cut -d' ' -f1)
